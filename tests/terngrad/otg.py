...
def test_time(input_size, bitwidth, random):
    ...
    while total_cnt < 10:
        ...
        mx.nd.contrib.OTG(data=input_data,bitwidth=bitwidth,random=random,out=output)
        output.wait_to_read()
        t2 = time.time()
        input_reverse = mx.nd.contrib.OTGr(data=output,bitwidth=bitwidth)
        t3 = time.time()
        total_time = total_time + t2 - t1
        total_time_reverse = total_time_reverse + t3 - t2
        if total_time + total_time_reverse > 10:
            break
